...
export function FinancingPanel({ carId, fillHeight = false }: { carId: string; fillHeight?: boolean }) {
  ...
  return (
    ...
    >
          ...
          {items.map((item) => {
            ...
            const progress =
              item.installmentCount > 0
                ...
                : 0;
            return (
              <div key={item.id} className="relative border border-slate-200 rounded-lg p-3 sm:p-4 hover:border-slate-300 transition-all">
                <div className="flex flex-col gap-3">
                  <div className="flex w-full min-w-0 items-start gap-3 pr-9 sm:flex-1">
                    <div
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="max-w-full truncate text-sm font-bold text-slate-900">{item.institution}</span>
                        <span
                ...
                <div className="absolute right-2 top-2 z-30" data-financing-menu={item.id}>
                  <button
                    type="button"
                    onClick={() => setOpenMenuId((current) => current === item.id ? null : item.id)}
                    className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700"
                    aria-label={`Abrir opções de ${item.institution}`}
                    aria-haspopup="menu"
                  ...
                  {openMenuId === item.id && (
                    <>
                      ...
                      <div
                        role="menu"
                        className="absolute right-0 top-9 z-30 w-32 overflow-hidden rounded-lg border border-slate-200 bg-white py-1 shadow-lg"
                      >
                        <button
                          type="button"
                          role="menuitem"
                          ...
                          className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:text-blue-600"
                        >
                ...
                {item.installmentCount > 0 && (
                  <div className="mt-3">
                    <div className="mb-1.5 flex flex-col gap-0.5 text-[11px] font-semibold text-slate-500 sm:flex-row sm:items-baseline sm:justify-between sm:gap-3">
                      <span>
      ...
      {showModal && (
        <div
          className="fixed inset-0 z-[80] flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-sm"
          onClick={() => setShowModal(false)}
        >
          <div
            className="bg-white rounded-lg shadow-2xl max-w-lg w-full p-4 sm:p-6 max-h-[90vh] overflow-y-auto"
            ...
            <form onSubmit={save} className="space-y-4 text-xs">
              ...
              <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-2 sm:flex-row sm:justify-end">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="w-full px-4 py-2.5 rounded-md text-sm font-semibold text-slate-600 hover:bg-slate-100 sm:w-auto"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex w-full items-center justify-center gap-2 rounded-md bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60 sm:w-auto"
                >
