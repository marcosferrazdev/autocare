        ...
        <div className={`space-y-3 ${fillHeight ? 'flex-1 min-h-0 overflow-y-auto overscroll-contain pr-1' : ''}`}>
          {schedules.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="relative flex items-start gap-3.5 py-4 pl-4 pr-12 border border-slate-200 rounded-md hover:shadow-sm transition-all"
              >
                ...
                <div className="absolute right-2 top-2 z-30" data-schedule-menu={s.id}>
                  <button
                    type="button"
                    onClick={() => setOpenMenuId((current) => current === s.id ? null : s.id)}
                    className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700"
                    aria-label={`Abrir opções de ${s.type}`}
                    aria-haspopup="menu"
