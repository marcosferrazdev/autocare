...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    const items = await UpgradeItemService.listByCar(carId, user.id);
    ...
    return NextResponse.json({ error: error.message || 'Erro ao carregar melhorias.' }, { status });
  }
}
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const result = UpgradeItemSchema.safeParse(body);
    ...
    const item = await UpgradeItemService.create(carId, user.id, result.data);
    return NextResponse.json(item, { status: 201 });
  } catch (error: any) {
    console.error('Erro ao cadastrar melhoria:', error);
    const status = error.message?.includes('permissão') || error.message?.includes('autorizado') ? 403 : 500;
